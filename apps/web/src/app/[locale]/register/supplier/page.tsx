...
const schema = z.object({
  ...
  primary_depot_name: z.string().min(2),
  primary_depot_address: z.string().min(2),
});
...
export default function SupplierRegisterPage() {
  ...
  async function onSubmit(values: FormValues) {
    setError(null);
    try {
      const res = await authApi.registerSupplier(values);
    ...
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-lg px-4 py-16">
        <Card className="animate-fade-in-up">
          <h1 className="text-2xl font-bold text-foreground">{t("supplierCardTitle")}</h1>
          <form onSubmit={handleSubmit(onSubmit)} className="mt-6 flex flex-col gap-4">
            ...
            <Input
              label={t("depotName")}
              {...register("primary_depot_name")}
              error={errors.primary_depot_name?.message}
            />
            <Input
              label={t("depotAddress")}
              {...register("primary_depot_address")}
              error={errors.primary_depot_address?.message}
            />
