...
7. **Phase 8 — Backend hardening**: promote the remaining mock/frontend-only data layers (products, orders, wallet, chat, scanning) to real FastAPI + PostgreSQL models and endpoints, one module at a time, reusing the auth backend's patterns (Alembic migrations, Pydantic schemas, async SQLAlchemy) from Phase 1.
