...
4. **Phase 5 — Moderated chat & wholesale**: conversation inbox/thread UI with the "pending review" state, agent message-moderation queue, wholesale request flow with escrow status (wires up the `credit_escrow_hold`/`credit_escrow_release` ledger entries reserved by Phase 4).
5. **Phase 6 — Warehouse scanning**: stock-in/boxing/handover/returns/transfer scan screens designed for both a hardware barcode scanner (keyboard-emulation input) and a camera-based PWA scan; also wires the mocked preparing/ready_for_pickup/handed_to_courier order-status transitions from Phase 3 to real scan events.
6. **Phase 7 — White-label store & admin settings**: seller store setup/billing/API keys, the public storefront (Section H), the remaining admin settings screens (commissions, couriers, roles matrix, audit logs), and `/admin/finance` (platform-wide finance oversight, deferred from Phase 4).
7. **Phase 8 — Disputes & post-delivery returns**: seller/agent/admin dispute pages (sitemap rows 32a/32b, 74a/74b, 101d) deferred from Phase 3, wiring the "Open Dispute" button on order detail to a real flow with evidence upload, agent decision, and refund via the wallet module from Phase 4 (`debit_dispute_refund`).
8. **Phase 9 — Backend hardening**: promote the remaining mock/frontend-only data layers (products, orders, wallet, chat, scanning) to real FastAPI + PostgreSQL models and endpoints, one module at a time, reusing the auth backend's patterns (Alembic migrations, Pydantic schemas, async SQLAlchemy) from Phase 1.
