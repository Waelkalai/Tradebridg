...
3. **Phase 4 — Wallet, retour balance & withdrawals**: wallet dashboard, withdrawal request flow (bank/cash) with OTP, retour balance page, manager withdrawal-approval queue — promotes the Phase 3 `lib/wallet.ts` stub into the real module.
...
7. **Phase 8 — Disputes & post-delivery returns**: seller/agent/admin dispute pages (sitemap rows 32a/32b, 74a/74b, 101d) deferred from Phase 3, wiring the "Open Dispute" button on order detail to a real flow with evidence upload, agent decision, and refund via the wallet module from Phase 4.
