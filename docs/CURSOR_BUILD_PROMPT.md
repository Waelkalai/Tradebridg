...
6. **Phase 7 — White-label store & admin settings**: seller store setup/billing/API keys, the public storefront (Section H), the remaining admin settings screens (commissions, couriers — making Phase 6's fixed mock courier list admin-configurable, roles matrix, audit logs), and `/admin/finance` (platform-wide finance oversight, deferred from Phase 4).
