...
> **تصحيح على الفكرة الأصلية:** كانت هناك تعارضات محتملة بين الحروف (استعمال A مرتين لـ Agent و Admin في نفس الوقت). الجدول أدناه هو **النسخة النهائية المعتمدة والموحَّدة** مع مخطط واجهات المنصة (Frontend Sitemap)، بحروف فريدة 100%: `S` بائع، `P` مورد/شحّان، `A` وكيل، `M` مدير وكالة، `D` أدمن/مدير عام.

| الدور | الحرف | التسمية الكاملة | من يُنشئ الحساب |
|---|---|---|---|
| بائع | **S** | Seller | يُنشئه بنفسه (تسجيل ذاتي) + تفعيل من وكيل |
| مورد/شحّان | **P** | Supplier/Shipper | يُنشئه بنفسه (تسجيل ذاتي) + تفعيل من وكيل |
...
- **صفحة تفصيل المنتج**: كل الصور، الفيديو، أسعار التفصيل والجملة بمستوياتها، الوصف، التوفر في كل وكالة (بدون كشف هوية المورد أبدًا — يظهر فقط "مورد #P-XXXX" كمرجع داخلي غير قابل للتواصل المباشر).
...
- لا يظهر اسم/هاتف/بريد/عنوان أي طرف للطرف الآخر أبدًا. يظهر فقط المعرّف (`S4821`, `P1190`).
...
| الصلاحية / الدور | Admin (D) | Manager (M) | Agent (A) — حسب التخصيص | Seller (S) | Supplier/Shipper (P) |
|---|---|---|---|---|---|
...
| **المورد/الشحّان (Supplier/Shipper - P)** | ينشئ حسابه بنفسه، يخضع للتفعيل من وكيل |
...
Seller / Supplier → تسجيل ذاتي عام (لا يحتاج إنشاء من طرف أحد) + تفعيل من Agent
```
...
| **Supplier/Shipper (P)** | طلبات الجملة المعلَّقة، مخزونه الافتراضي والفعلي بكل وكالة، مبيعات منتجاته (بدون كشف هوية البائعين)، رصيد محفظته |
